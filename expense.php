...
<body>
  ...
  <header>
    ...
    <ul class="menu">
      ...
      <li><a class="selected-option" href="expense.php">Add Expense</a></li>
  ...
  <main>
    ...
    <form class="form" action="expense_action.php" method="post">
      ...
      <select name="category" type="text" class="form-control form-middle-elem text-centered" placeholder="Select category" required="">
        ...
        <?php
          ...
          $stmt = "SELECT name FROM user_expense_category WHERE user_id='$userId'";
      ...
      <select name="payment" type="text" class="form-control form-middle-elem text-centered" placeholder="Select payment" required="">
        ...
        <?php
          // get user payment methods from db
          $payments = array();
          require_once("db.php");
          $userId = $_SESSION['userId'];
          $stmt = "SELECT name FROM user_payment_method WHERE user_id='$userId'";
          $query = $conn->query($stmt);
          while ($result = $query->fetch(PDO::FETCH_ASSOC))
          {
            array_push($payments, $result['name']);
          }
          natcasesort($payments); // order natural, case insensitive
      ...
      <div class="grid-container">
        ...
        <button name="add" class="confirm right btn btn-lg btn-primary btn-info " type="submit">Add expense</button>
      </div>
    </form>

    <?php
      if (isset($_SESSION['msg']) && $_SESSION['msg']=='expense_added')
      {
        echo '<div style="color: green; font-size: 25px;">'."Expense added".'</div>';
        unset($_SESSION['msg']);
      }
    ?>
